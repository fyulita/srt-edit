def clearFormat(line):
    b, b_end, i, i_end, u, u_end, f, fc, p = 0, 0, 0, 0, 0, 0, 0, 0, 0
    while b + b_end + i + i_end + u + u_end + f + fc + p != -9:
        # Clear bold letters
        b = line.find("<b>")
        if b == -1:
            b = line.find("{b}")
        if b != -1:
            line = line[:b] + line[b + 3:]

        b_end = line.find("</b>")
        if b_end == -1:
            b_end = line.find("{/b}")
        if b_end != -1:
            line = line[:b_end] + line[b_end + 4:]
        ...
        if u == -1:
            u = line.find("{u}")
        if u != -1:
            line = line[:u] + line[u + 3:]

        u_end = line.find("</u>")
        if u_end == -1:
            u_end = line.find("{/u}")
        if u_end != -1:
            line = line[:u_end] + line[u_end + 4:]
